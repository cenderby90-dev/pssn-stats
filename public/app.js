...
function renderLeaderboard(filter) {
  ...
  if (!ranked.length) {
    lbEl.innerHTML = `<div style="padding:1.5rem;color:var(--muted);font-size:0.85rem;">No results for this format yet.</div>`;
    return;
  }
...
function renderCountdown() {
  ...
  if (!upcoming.length) { el.innerHTML = `<div style="font-size:0.85rem;color:var(--muted);">No upcoming events found.</div>`; return; }
